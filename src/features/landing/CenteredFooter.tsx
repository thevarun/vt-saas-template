...
export const CenteredFooter = (props: {
  ...
  return (
    <div className="flex flex-col items-center text-center">
      ...
      <ul className="mt-4 flex gap-x-8 text-lg max-sm:flex-col [&_a:hover]:text-foreground [&_a]:text-muted-foreground">
        {props.children}
      </ul>
      ...
      <div className="mt-6 flex w-full items-center justify-between gap-y-2 border-t pt-3 text-sm text-muted-foreground max-md:flex-col">
        ...
        <ul className="flex gap-x-4 font-medium [&_a:hover]:text-foreground [&_a]:text-muted-foreground">
          {props.legalLinks}
        </ul>
      </div>
    </div>
