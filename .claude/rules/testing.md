...
- **Stories are tests.** Every `*.stories.tsx` runs as a Vitest browser test via `pnpm test:stories` (Storybook Vitest addon, headless Chromium) — a broken import, render error, or failing play function fails CI. It's a separate opt-in project (`VITEST_STORYBOOK=1`); keep it out of the default `pnpm test` so that run stays browser-free. Add a story when you add a shared `components/ui/*` primitive.
