...
To execute all ~35 production-readiness checks across all audit domains by dispatching parallel sub-agents. Each sub-agent handles a group of related domains and returns structured results. This step runs fully autonomously and auto-proceeds to report generation.
...
Launch all 6 sub-agents in parallel using the Task tool. Each agent receives:
...
MCP available - GitHub: {github_mcp} | Vercel: {vercel_mcp}
